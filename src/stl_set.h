...
public:
	...
	set() :m_rbt(Compare()) {}
	explicit set(const Compare& c) :m_rbt(c) {}
	
	set(const value_type* first, const value_type* last) :m_rbt(Compare()) {
		m_rbt.insert_unique(first, last);}

	set(const iterator first, const iterator last) :m_rbt(Compare()) {
		m_rbt.insert_unique(first, last);}

	set(const iterator first, const iterator last, Compare&c) :m_rbt(c) {
		m_rbt.insert_unique(first, last);
	}

	set(set& s) :m_rbt(s.m_rbt) {}
	...
	inline set& operator=(const set& x) {
		m_rbt = x.m_rbt;
		return*this;
	}
...
inline bool operator==(const set<Key, Compare,Alloc>&s1,
	                   const set<Key, Compare,Alloc>&s2){
